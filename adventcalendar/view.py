...
class SetRewardsView(discord.ui.View):
    ...
    async def get_embed(self) -> discord.Embed:
        embed: discord.Embed = discord.Embed(
            title=_("🎄 Final (24 boxes opened) 🎄") if self.current_day is None else _("🎄 Rewards for Day {day} 🎄").format(day=self.current_day),
            color=await self.ctx.embed_color(),
        )
        embed.set_thumbnail(url="attachment://christmas_tree.png")
        day_rewards = await self.cog.config.guild(self.ctx.guild).rewards.get_raw(str(self.current_day) if self.current_day is not None else "null")
        for reward in sorted(day_rewards, key=lambda r: (r["priority"], ("role", "temp_role", "bank_credits", "levelup_xp", "custom", "message", None).index(r["type"]))):
            if reward["type"] == "role":
                ...
                if role is None:
                    continue
                embed.add_field(
                    name=_("• Role Reward ({priority} ; {percent}{multiplied_priority_percent})").format(
                        ...
                        multiplied_priority_percent=(
                            f" ; x{reward['priority_multiplier']} {round(reward['priority'] * reward['priority_multiplier'] / sum([r['priority'] * r['priority_multiplier'] for r in day_rewards]) * 100, 2)}%"
                ...
                if role is None:
                    continue
                embed.add_field(
                    name=_("• Temp Role Reward ({priority} ; {percent}%{multiplied_priority_percent})").format(
                        priority=reward["priority"],
                        percent=round(reward["priority"] / sum([r["priority"] for r in day_rewards]) * 100, 2),
                        multiplied_priority_percent=(
                            f" ; x2 {round(reward['priority'] * reward['priority_multiplier'] / sum([r['priority'] * r['priority_multiplier'] for r in day_rewards]) * 100, 2)}%"
                            if reward["priority_multiplier"] is not None
                            else ""
                        ),
                    ),
                ...
                embed.add_field(
                    name=_("• Bank Credits Reward ({priority} ; {percent}%{multiplied_priority_percent})").format(
                        priority=reward["priority"],
                        percent=round(reward["priority"] / sum([r["priority"] for r in day_rewards]) * 100, 2),
                        multiplied_priority_percent=(
                            f" ; x{reward['priority_multiplier']} {round(reward['priority'] * reward['priority_multiplier'] / sum([r['priority'] * r['priority_multiplier'] for r in day_rewards]) * 100, 2)}%"
                ...
                embed.add_field(
                    name=_("• LevelUp XP Reward ({priority} ; {percent}%{multiplied_priority_percent})").format(
                        priority=reward["priority"],
                        percent=round(reward["priority"] / sum([r["priority"] for r in day_rewards]) * 100, 2),
                        multiplied_priority_percent=(
                            f" ; x{reward['priority_multiplier']} {round(reward['priority'] * reward['priority_multiplier'] / sum([r['priority'] * r['priority_multiplier'] for r in day_rewards]) * 100, 2)}%"
                ...
                embed.add_field(
                    name=_("• Custom Reward ({priority} ; {percent}%{multiplied_priority_percent})").format(
                        priority=reward["priority"],
                        percent=round(reward["priority"] / sum([r["priority"] for r in day_rewards]) * 100, 2),
                        multiplied_priority_percent=(
                            f" ; x{reward['priority_multiplier']} {round(reward['priority'] * reward['priority_multiplier'] / sum([r['priority'] * r['priority_multiplier'] for r in day_rewards]) * 100, 2)}%"
                ...
                embed.add_field(
                    name=_("• Message ({priority} ; {percent}%{multiplied_priority_percent})").format(
                        priority=reward["priority"],
                        percent=round(reward["priority"] / sum([r["priority"] for r in day_rewards]) * 100, 2),
                        multiplied_priority_percent=(
                            f" ; x{reward['priority_multiplier']} {round(reward['priority'] * reward['priority_multiplier'] / sum([r['priority'] * r['priority_multiplier'] for r in day_rewards]) * 100, 2)}%"
                ...
                embed.add_field(
                    name=_("• Nothing ({priority} ; {percent}%{multiplied_priority_percent})").format(
                        priority=reward["priority"],
                        percent=round(reward["priority"] / sum([r["priority"] for r in day_rewards]) * 100, 2),
                        multiplied_priority_percent=(
                            f" ; x{reward['priority_multiplier']} {round(reward['priority'] * reward['priority_multiplier'] / sum([r['priority'] * r['priority_multiplier'] for r in day_rewards]) * 100, 2)}%"
    ...
    async def role(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        await interaction.response.send_modal(
            RoleRewardModal(self.cog, self.current_day, self)
        )
    ...
    async def bank_credits(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        await interaction.response.send_modal(
    ...
    async def custom_reward(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        await interaction.response.send_modal(
            CustomRewardModal(self.cog, self.current_day, self)
        )
    ...
    async def message(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        await interaction.response.send_modal(
            MessageModal(self.cog, self.current_day, self)
        )
    ...
    async def nothing(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        await interaction.response.send_modal(
            NothingModal(self.cog, self.current_day, self)
        )
    ...
    async def clear(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        ...
        if not await CogsUtils.ConfirmationAsk(
            fake_context,
            (
                _("Are you sure you want to clear all rewards for day {day}?").format(day=self.current_day)
            ...
            return
        await self.cog.config.guild(self.ctx.guild).rewards.clear_raw(str(self.current_day) if self.current_day is not None else "null")
        await self._update()
...
class PriorityModal(discord.ui.Modal):
    def __init__(self, cog: commands.Cog, day: typing.Optional[int], view: SetRewardsView, *, title: str) -> None:
...
class RoleRewardModal(PriorityModal):
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        ...
        try:
            ...
            return
        await interaction.response.defer()
        day_rewards = await self.cog.config.guild(self.view.ctx.guild).rewards.get_raw(str(self.day) if self.day is not None else "null")
        day_rewards.append(
            {
                "type": "role",
                "priority": priority,
        ...
        await self.cog.config.guild(self.view.ctx.guild).rewards.set_raw(str(self.day) if self.day is not None else "null", value=day_rewards)
