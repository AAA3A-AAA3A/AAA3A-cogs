    ...
    async def list(self, ctx: commands.Context, message: discord.Message = None) -> None:
        ...
        for li in lists:
            ...
            for dropdown_text in li:
                ...
                value += "\n".join([f"• `{config_identifier}` - Emoji {ctx.bot.get_emoji(int(data['emoji'])) if data['emoji'].isdigit() else data['emoji']} - Label `{data['label']}` - Text `{data['text'].replace(break_line, ' ')}`" for config_identifier, data in dropdown_text.items() if config_identifier != "message"])
                embed.add_field(
