    ...
    async def load_dropdowns(self):
        ...
        for guild in all_guilds:
            ...
            for dropdown_text in all_guilds[guild]["dropdowns_texts"]:
                try:
                    self.bot.add_view(Dropdown(timeout=None, placeholder=_("Select an option."), min_values=0, max_values=1, options=self.get_dropdown(config, dropdown_text), function=self.on_dropdown_interaction, infinity=True, custom_id=f"DropdownText_{dropdown_text}"), message_id=int((str(dropdown_text).split("-"))[1]))
