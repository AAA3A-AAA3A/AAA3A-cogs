...
class JoinGameView(discord.ui.View):
    ...
    async def start(self, ctx: commands.Context) -> discord.Message:
        ...
        embed: discord.Embed = discord.Embed(
            ...
            description=_(
                "Click the button below to join the party! Please note that the maximum amount of players is 15."
            ),
    ...
    async def view_players(
        ...
        if not self.players:
            ...
            return
        embed = discord.Embed(
            title=_("Russian Roulette Game — Players"),
            color=await self.ctx.embed_color(),
        )
        embed.set_author(
            name=_("Hosted by {host.display_name}").format(host=self.host),
...
class ShotView(discord.ui.View):
    def __init__(self, cog: commands.Cog, player: discord.Member) -> None:
