    ...
    async def convert(self, ctx: commands.Context, argument: str) -> Team:
        ...
        try:
            ...
            if ctx.guild.id in cog.teams:
                if (team := discord.utils.get(cog.teams[ctx.guild.id].values(), name=argument)) is not None:
                    return team
                if (team := discord.utils.get(cog.teams[ctx.guild.id].values(), display_emoji=argument)) is not None:
                    return team
...
class TeamOrMemberConverter(commands.Converter):
    async def convert(self, ctx: commands.Context, argument: str) -> typing.Union[Team, discord.Member]:
