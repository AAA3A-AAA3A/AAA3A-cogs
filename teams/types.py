...
class Point:
    ...
    def get_member(self, guild: discord.Guild) -> discord.Member | None:
        if self.member_id is None:
            return None
        return guild.get_member(self.member_id)

    def get_managed_by(self, guild: discord.Guild) -> discord.Member | None:
...
class Team:
    ...
    captain_id: int
    vice_captain_ids: list[int] = field(default_factory=list)
    member_ids: list[int] = field(default_factory=list)
    points: list[Point] = field(default_factory=list)

    emoji: str | int | None = None
    ...
    vice_captain_role_id: int | None = None
    ...
    @property
    def captain(self) -> discord.Member | None:
    ...
    @property
    def vice_captains(self) -> list[discord.Member]:
        if (guild := self.guild) is None:
            return []
        return [
            vice_captain
    ...
    def display_emoji(self, emoji: discord.Emoji | str | None) -> None:
    ...
    @color.setter
    def color(self, color: discord.Color | None) -> None:
    ...
    @property
    def captain_role(self) -> discord.Role | None:
        if (guild := self.guild) is None:
            return None
        return guild.get_role(self.captain_role_id)

    @captain_role.setter
    def captain_role(self, role: discord.Role | None) -> None:
        self.captain_role_id = role.id if role is not None else None

    @property
    def vice_captain_role(self) -> discord.Role | None:
        if (guild := self.guild) is None:
            return None
        return guild.get_role(self.vice_captain_role_id)

    @vice_captain_role.setter
    def vice_captain_role(self, role: discord.Role | None) -> None:
    ...
    @member_role.setter
    def member_role(self, role: discord.Role | None) -> None:
    ...
    async def remove_member(self, member: discord.Member) -> None:
        ...
        if member.guild.me.guild_permissions.manage_roles:
            ...
            ) is not None and vice_captain_role in member.roles:
                try:
                    await member.remove_roles(
                        vice_captain_role,
                        reason="Removing vice captain role from team member.",
                    )
                except discord.HTTPException:
                    pass
            if (member_role := self.member_role) is not None and member_role in member.roles:
                try:
                    await member.remove_roles(
                        member_role,
                        reason="Removing member role from team member.",
                    )
                except discord.HTTPException:
                    pass
        ...
        if (
            ...
            try:
                await member.add_roles(
                    vice_captain_role,
                    reason="Adding vice captain role to team member.",
                )
            except discord.HTTPException:
                pass
    ...
    async def demote_member(self, member: discord.Member) -> None:
        ...
        if member.id not in self.vice_captain_ids:
            ...
            if captain_role not in new_captain.roles:
                try:
                    await new_captain.add_roles(
                        captain_role,
                        reason="Adding captain role to new team captain.",
                    )
                except discord.HTTPException:
                    pass
            if old_captain is not None and captain_role in old_captain.roles:
                try:
                    await old_captain.remove_roles(
                        captain_role,
                        reason="Removing captain role from old team captain.",
                    )
                except discord.HTTPException:
                    pass
    ...
    async def add_points(
        ...
        managed_by: discord.Member | None = None,
    ) -> None:
        self.points.append(
            Point(
                amount=amount,
