...
class TransferChannel(commands.Cog):
    ...
    if CogsUtils().is_dpy2:
        @transferchannel.command()
        async def user(self, ctx: commands.Context, source: discord.TextChannel, destination: discord.TextChannel, way: commands.Literal["embeds", "webhooks", "messages"], user: discord.User, limit: typing.Optional[int]=None):
            """Transfer a part of a channel's messages channel in a other channel. This might take a long time.
